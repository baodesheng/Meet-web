 ...
 * Date: 13-11-8
 * Time: 上午10:21
 * 此脚本用于把LIBA_CREAM文章定时导入到纪录片2.0的文章数据结构里
 */
...
function getCream(queryParams) {
    ...
    var creamNext = function(msg, r){
        if (r && r instanceof Array && r.length > 0) {
                ...
                var userNext = function(msg, rs) {
                    if (rs && JSON.parse(rs).id && !isNaN(JSON.parse(rs).id)) {
                        ...
                        var articleJSon = {
                            ...
                            "status": 3,//0:初始 1：直播 2:精选 3:预备精选 4:废弃
            ...
            console.log("crontab import finish!");
        }
    };

    var creamSql = "select * from " + TABLE_NAME_CREAM + " where status=0 and topic_id > 0 and id > " + autoCreamId + " order by id";
