	...
	return this.each(function(){
		...
		if($this.data('promptumenu')){
			...
			$this.children().each(function(i){
				...
				var $imgBox= $li.children(".img-box");
				var $img = $imgBox.children("img");
				var len = $imgBox.length;
				var imgBoxHeight = $(".img-box").height();
				if(len==0){					
					$text.css("margin-top",(properties.height-$text.height())/2);
				}else{
					//$img.css("margin-top",(imgBoxHeight-$img.height())/2);
				}				
				...
				if(settings.direction == 'vertical'){
					...
					$li.css({
						'top': 0,//Math.round((cursor.y * cells.height - cells.height/2) - ($li.height()/2)),
						'left': li_left
					});
